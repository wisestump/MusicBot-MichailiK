...
public class RemoveCmd extends MusicCommand 
{
    public RemoveCmd(Bot bot)
    {
        ...
        this.bePlaying = true;
        this.options = Collections.singletonList(new OptionData(OptionType.INTEGER, "position", "Queue position", true));
    }
    ...
    public void doCommand(SlashCommandEvent event)
    {
        ...
        else if(isDJ)
        {
            ...
            event.reply("Removed **"+qt.getTrack().getInfo().title
                    +"** from the queue (requested by <@"+qt.getIdentifier()+">")
            .allowedMentions(Collections.emptyList())
            .queue();
        }
        else
        {
            event.reply(getClient().getError()+" You cannot remove **"+qt.getTrack().getInfo().title+"** because you didn't add it!")
                .setEphemeral(true)
                .queue();
        }
    }
